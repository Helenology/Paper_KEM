...
def get_original_data_newversion(lung_image_path, lung_mask_path, sample_CT_path):
    ...
    Generate the true parameters for the simulation study.
...
def generate_pi_new_version(sample_CT_array, lung_mask_array, bone_threshold=400, IsShow=False):
    ...
    CT_shape = sample_CT_array.shape  # (depth, width, height)
    pi_shape = (K,) + CT_shape  # (K=3, depth, width, height)
    voxel_class = np.zeros(shape=pi_shape)  # the real classes

    ######################## voxel class ########################
    # class 1 (bone): if value >= bone_threshold, then voxel_class(bone)[1] = 1;
    #                 otherwise voxel_class(bone)[1] = 0
    voxel_class[1] = 1 * (sample_CT_array >= bone_threshold)
    # class 2 (lung): if lung mask = 1, then voxel_class(lung)[2] = 1;
    #                 otherwise voxel_class(lung)[2] = 0
    ...
    ######################## pi ########################
    # (K, depth, width, height, 1)
    voxel_class_tf = tf.reshape(tf.convert_to_tensor(voxel_class, dtype=tf.float32), pi_shape + (1,))
    # local average the classes as the prior probability
    pi_tf = AveragePooling3D(pool_size=(3, 3, 3), strides=1, padding="same")(voxel_class_tf)

    # avoid too many 0s and 1s in pi
    smoothness = tf.reshape([0.6, 0.6, 0.6], (3, 1, 1, 1, 1))
    pi_tf += smoothness
    pi_tf /= tf.reduce_sum(pi_tf, axis=0)  # sum pi should be 1
    voxel_class_tf = tf.squeeze(voxel_class_tf)  # (K, depth, width, height)
    pi_tf = tf.squeeze(pi_tf)  # (K, depth, width, height)
...
def generate_mu_sigma_new_version(sample_CT_array, voxel_class, IsShow=False):
    ...
    CT_max = sample_CT_array.max().astype(np.float64)  # the max value
    CT_min = sample_CT_array.min().astype(np.float64)  # the min value
    ...
    mu_const0 = 1
    # class bone (class 1)
    data1 = sample_CT_array[voxel_class[1] == 1]
    mu_const1 = np.mean(data1)  # global average
    # class lung (class 2)
    data2 = sample_CT_array[voxel_class[2] == 1]
    mu_const2 = np.mean(data2)  # global average
    ...
    # generate sigma
    sigma_const1 = np.std(data1)
    sigma_const2 = np.std(data2)
...
def generate_simulate_data_new_version(pi, mu, sigma, seed=0, IsShow=False):
    ...
    :param mu: the mean of K classes
    :param sigma: the standard deviation of K classes
    ...
    simulate_data = tf.zeros(shape=CT_shape)  # empty simulate data
    ...
    pi_realization = tf.transpose(pi_realization)  # in the original order
    assert np.all((pi_realization == 0) | (pi_realization == 1))  # each one-hot label is either 0 or 1

    pi_realization = tf.convert_to_tensor(pi_realization, dtype=tf.float32)  # in tensor form
    for k in range(K):
